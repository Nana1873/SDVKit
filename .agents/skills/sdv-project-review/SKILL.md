...
Transport one line at a time through `project review command`, with the required role for `network-2`. `press` accepts one exact SMAPI `SButton` name, such as `F8`, `Enter`, `MouseLeft`, `ControllerA`, or `DPadDown`, releases it on the next input tick, and permits both SMAPI's input update and Stardew's own background menu-input path only for the bounded dispatch interval. Button and cursor commands may run on title, loading, and error screens before `WorldReady`; they do not waive exact process, role, staging, AlwaysOn, or target-load checks. Set and confirm the process-local virtual cursor before any mouse-button press; it must never fall back to the physical pointer. `MouseWheelUp` and `MouseWheelDown` additionally require that cursor over an active menu. `cursor` enables the override only at a coordinate inside the current `Game1.uiViewport`; neither action may focus a window or move the user's physical pointer. Every action except clear fails closed unless the background-input adapter is installed and ready. Clear the override explicitly when the mouse path is complete. A successful AlwaysOn result proves the input was injected or the virtual coordinate was set; prove the intended target-mod effect separately through state, logs, or a viewport screenshot.

For explicitly authorized native MCP input, restart the selected role's server with `--allow-input`. This advertises only `stardew_input_press`, `stardew_input_cursor_set`, `stardew_input_cursor_clear`, and `stardew_input_wheel`; it does not expose the generic console command or authorize fixture actions. Each tool is one-shot, role-fixed, closed-schema, acknowledged, and rejected rather than queued while another MCP action owns that role. Require a stable published foreground-window identity and a status timestamp/game tick newer than the acknowledgement before accepting a result. Never automatically retry a cancellation or failure after dispatch may have occurred. A cancellation after a valid acknowledgement retains that acknowledgement with `cancellationRequested=true` and still requires the post-action binding check. Clear the virtual cursor after a mouse path; server EOF also attempts a bounded clear when that MCP session may have dispatched input and exits nonzero if cleanup is unconfirmed.

Every automated review mouse path must first verify the exact review ownership and topology role, use only this existing process-local SDVKit virtual cursor, and fail closed. Never use global `SendInput`, physical cursor movement, window-focus changes, or generic computer-use automation for review mouse input.
