...
and provider config before launch, preserving CP-generated bytes: CP can rewrite
valid config files to normalize formatting and add defaults. If an initial launch
does that, the existing staging-drift guard rejects refresh. First preserve the
generated staged config files in an ignored `.sdvkit/` working/evidence folder,
then stop/reset that review (single stop removes staging). Deliberately adopt the
preserved config bytes in your selected sources and start again.
Refresh itself never copies config back to source. Keep the SMAPI console idle. Edit your source
