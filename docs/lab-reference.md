...
AlwaysOn disables `pauseWhenOutOfFocus` in its isolated profile while running. On controlled exit it attempts to restore and read back the captured option, emits `exiting` or `restoreFailed`, and asks the game to exit normally in either case. An unconfirmed restoration is a warning after otherwise safe exit; the next start reapplies the lab option. Crashes or manual/forced termination do not promise restoration.
