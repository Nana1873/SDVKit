...
In a world-ready owned review, native mod controls reading the parameterless XNA `Mouse.GetState()` during the same player update also receive the confirmed virtual coordinates, wheel counter, and owned mouse buttons. This includes the update callbacks after SMAPI assembles input. The sample is bound to the exact review, player, input instance, local screen, and update thread; it ends when that player update returns or throws. Hardware-overlap checks retain the original physical sample. Cancellation and clear immediately remove the native override while retaining the consumed neutral wheel offset. Reads outside that update scope and other getter overloads retain their original behavior; verify the actual control effect for the selected mod.
